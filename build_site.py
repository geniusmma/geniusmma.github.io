...
def project_row(p,level=3,pre='',filterable=False,home=False):
    ...
    summaries={'us-construction-support':'RFIs, takeoffs, procurement tracking and estimating support for U.S. construction projects.','infrastructure-estimating':'Pre-tender estimates and cost plans for transport and utilities assignments.','weekly-snapshots':'Automated weekly snapshots that preserve reporting history in SharePoint and Power BI.'}
...
def company_row(c,level=3,pre='',home=False):
    ...
    period={'arcadis':'2019–Present','transitional-builders':'Ended Sep 2026 · ~5 months','workstaff360':'Completed engagement'}.get(c['id'],c['period']) if home else c['period']
    return f'''<article class="company-row"><div class="company-title"><h{level}><a href="{pre}companies/{c['id']}.html">{E(title)} {ARROW}</a></h{level}><span class="period">{E(period)}</span></div><p class="role">{E(ROLE[c['id']])}</p><p class="row-summary">{E(SUMMARY.get(c['id'],c['summary']))}</p></article>'''
...
about=f'''<main id="main" class="container page-main reading-page"><header class="page-heading"><h1>About me</h1></header><div class="about-intro"><img class="about-avatar" src="assets/img/Dexter.jpg" alt="Michael’s illustrated profile avatar" width="96" height="96"><div><p>I’m Michael Asistin, based in San Pablo City, Laguna, Philippines.</p><p>My work connects construction coordination, estimating and quantity surveying with reporting and automation. I’ve worked across site delivery, commercial support and remote construction teams, and currently work in business intelligence and automation at Arcadis.</p></div></div><section class="content-section"><h2>How I work</h2><p>I focus on clear records, careful checks and practical follow-through—whether I’m reviewing quantities, coordinating project documents or tracing a reporting issue.</p></section><section class="content-section" id="credentials"><h2>Qualifications</h2><ul class="credentials">{creds}</ul></section><section class="content-section" id="skills"><h2>Tools I use</h2><p><strong>Construction:</strong> Procore, ProjectHQ, Bluebeam, PlanSwift, Excel and SharePoint.</p><p><strong>Reporting & automation:</strong> Power BI, Power Query, DAX, Power Automate, Smartsheet, Power Apps and VBA.</p></section><section class="content-section" id="faq"><h2>Working together</h2><p>I’m open to job opportunities, freelance work and project-based engagements. Email me with the role or scope you have in mind. My phone number is available on request.</p><div class="intro-actions"><a class="button" href="mailto:{E(P['email'])}">Get in touch {ARROW}</a><a class="quiet-link" href="resume.pdf" download="Michael-Asistin-CV.pdf">Download CV ↓</a></div></section></main>'''
write('profile.html','About','Michael Asistin’s background, qualifications, tools and approach.',about,'about')

for p in ordered:
    c=CS[p['companyId']]
    tools=tags(p['tools']) if p['tools'] else ''
    ...
    body=f'''<main id="main" class="container page-main reading-page"><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../projects.html">← All work</a><a href="../companies/{c['id']}.html">{E(SHORT.get(c['id'],c['name']))}</a></nav><header class="page-heading project-heading"><p class="eyebrow">{E(p['category'])}</p><h1>{E(p['title'])}</h1><p>{E(p['summary'])}</p></header><dl class="project-facts"><div><dt>Role</dt><dd>{E(p['role'])}</dd></div><div><dt>Period</dt><dd>{E(p['period'])}</dd></div><div><dt>Status</dt><dd>{E(p['status'])}</dd></div></dl><section class="content-section" id="contribution"><h2>My contribution</h2>{bullets(p['contributions'])}</section><section class="content-section" id="deliverables"><h2>Deliverables</h2>{bullets(p['deliverables'],'deliverables')}</section><section class="content-section" id="status"><h2>Outcome & status</h2><p>{E(p['outcome'])}</p>{nextsteps}</section><div class="details-group"><details class="disclosure" id="overview"><summary>Project context</summary><p>{E(p['context'])}</p>{privacy}</details><details class="disclosure" id="tools"><summary>Tools & process</summary>{tools}{workflow}</details></div><p class="small page-note">Status as of {DATE}.</p>{related_section}</main>'''
...
for c in D['companies']:
    ...
    tools=f'<details class="disclosure" id="tools"><summary>Tools used</summary>{tags(c["tools"])}</details>' if c['tools'] else ''
    ...
    body=f'''<main id="main" class="container page-main reading-page"><nav class="breadcrumb" aria-label="Breadcrumb"><a href="../work.html">← All experience</a></nav><header class="page-heading"><h1>{E(c['name'])}</h1><p>{E(c['summary'])}</p></header><dl class="project-facts"><div><dt>Role</dt><dd>{E(c['role'])}</dd></div><div><dt>Period</dt><dd>{E(c['period'])}</dd></div><div><dt>Status</dt><dd>{E(c['status'])}</dd></div></dl><section class="content-section" id="scope"><h2>Responsibilities</h2>{bullets(c['scope'])}</section>{tools}<section class="content-section" id="projects"><h2>Selected work</h2>{privacy}<div class="work-list">{projects_html}</div></section></main>'''
